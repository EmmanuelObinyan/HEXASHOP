...
const Checkout = () => {
  const{discountPrice,cart,setFee}=useCart()
  ...
  const[shippingfee,setShippingFee]=useState(0)
   ...
   const totalFee=shippingfee + discountPrice
    setFee(shippingfee)
  return (
    <div className='border-1  border-gray-200 p-3 xs:w-[85%] sm:w-[65%] md:w-[50%] flex flex-col justify-self-center h-65 rounded-2xl bg-gray-50'>
          <span className='flex justify-between p-2 mt-5 '>
            {/* for the item price and its quantity in cart */}
             ...
             <p className='font-semibold text-gray-600 xs:text-sm sm:text-sm md:text-md lg:text-xl '>{`${option === 'NGN'? "NGN":"$"} ${option === "NGN" ? Math.round(currencyData * discountPrice) : discountPrice.toFixed(2)}`}</p>
          </span>
          <span className='flex justify-between p-2 mt-3 '>
           ...
           <span className='flex justify-between p-2 mt-5 '>
             ...
             <p className='font-semibold text-gray-800 xs:text-sm sm:text-sm md:text-lg lg:text-2xl '>{`${option === 'NGN' ? "NGN":"$"} ${option === 'NGN' ? Math.round(currencyData * totalFee) :totalFee.toFixed(2)}`}</p>
          </span>
