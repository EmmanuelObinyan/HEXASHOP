...
const CartProduct = () => {
  ...
  return (
    <>
     { 
         cart.map((item)=>(
      <div className="h-fit flex border-1 border-gray-300  p-2 xs:flex-col lg:flex-row" key={item.id}>
        {/* first section */}
        <div className=" xs:w-[full] sm:w-[12rem] lg:w-[17.5rem] sm:mx-auto lg:mx-0">
          <img src={item.image } alt="" className="xs:h-55 sm:h-50 lg:h-60 object-cover object-top w-full " />
        </div>
        {/* second section */}
        <div className="md:h-50 lg:h-60 flex justify-between xs:flex-col lg:flex-row ">
          ...
          <div className="md:w-[full] lg:w-[fit-content] flex flex-col justify-around">
            ...
            {/* for the price */}
            <p className="xs:text-md md:text-xl lg:text-2xl text-gray-700 p-2 font-semibold ">${item.price * item.quantity}</p>
          </div>
          ...
          <div className="xs:w-[15rem] sm:w-[14rem] md:w-[15rem] lg:w-[fit-content] flex xs:mx-auto md:ml-15 justify-between lg:flex-col lg:justify-around ml-35">
            ...
            <FaRegTrashAlt className="text-4xl lg:mx-auto font-bold p-2 text-red-500 "  onClick={() => deleteCart(item.id)}  />
            <button className="xs:w-24 md:w-25 lg:w-30 p-2 border-1 border-gray-300 text-gray-800 rounded-lg flex items-center bg-gray-100 justify-center">
             ...
             <FaPlus className="md:text-lg lg:text-xl font-bold " onClick={()=>increaseQuantity(item)}/>
              <span className="md:text-md lg:text-lg font-bold mx-3">{item.quantity }</span>
              {/* for the decrease quantity of the cart */}
              <FaMinus className="md:text-lg lg:text-xl font-semi " onClick={() => decreaseQuantity(item)} />
            </button>
          </div>
        </div>
      </div>
