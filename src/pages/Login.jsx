...
const Login = () => {
  ...
  return (
    <div className="flex h-screen xs:flex-col sm:flex-col md:flex-row">
      {/* first container */}
      <div
        className='flex-1 flex flex-col !bg-cover justify-center xs:pb-2 sm:pb-3
      text-white bg-center bg-no-repeat bg-[linear-gradient(to_left,rgba(0,0,0,0.9),rgba(0,0,0,0.5)),url("/womanlogin.png")]'
      >
        {/* first coloumn */}
        <h2 className=" ml-4 capitalize w-fit p-3 font-medium xs:text-3xl sm:text-3xl md:text-4xl lg:text-6xl mt-45">
          welcome to hexashop
        </h2>
        <p className="ml-4 pt-2 xs:text-[0.8rem] sm:text-[0.8rem] md:text-[0.9rem]  lg:text-[1.2rem] w-[90%]">
          we have different quality clothes ,and assesories.patronize us and get
          the best of our products and see for your self, lorem ipsum and aother
          things can be gotten from here too
        </p>
      ...
      <div className="flex-1 ">
        ...
        <figure className="w-fit flex items-center  gap-1 justify-self-center xs:mt-8 sm:mt-10  md:mt-15 lg:mt-18 p-2">
          <img
            ...
            alt=""
            className={` ${
              dark ? "brightness-90 invert" : "*:"
            } xs:h-10 sm:h-12 w-[2.8rem] md:w-[3rem]  lg:w-[4rem] h-20`}
          />
          <img
            src={secondpart}
            alt=""
            className={`sm:h-9 ${
              dark ? "brightness-90 invert" : ""
            } md:h-10 md:w-[7rem] lg:w-[9rem] lg:h-13`}
          />
        </figure>
        <Form
          data={user}
