...
const PaymentPage = () => {
  ...
  const totalPrice= ` ${option === 'NGN' ? 'NGN':"$"} ${option === 'NGN' ? Math.round(currencyData * total):total}`
    ...
    <Toaster/>
      ...
      >
        ...
        <section className="mt-8 xs:w-[95%] sm:w-[80%] md:w-[70%] mx-auto">
          ...
          <p className={`flex  items-center justify-between xs:text-md sm:text-lg md:text-xl lg:text-2xl font-medium capitalize transition-all ease duration-200 ${ dark ? 'text-white':'text-gray-400' } `}>
            customer address
            <span
          ...
          {deliveryAddress.map((item, index) => (
            <div
              className={` ${dark ? "":"bg-gray-50"} border-1 border-gray-200 pb-2 xs:h-20 md:h-17 lg:h-18 px-3  `}
              key={index}
            >
          ...
          <p className={`flex  items-center justify-between xs:text-md sm:text-lg md:text-xl lg:text-2xl font-medium capitalize  transition-alll duration-200 ease ${ dark ? "text-white":"text-gray-400" }`}>
            delivery details
            <span
              className=" mr-1 xs:text-xs md:text-sm lg:text-md text-orange-400 font-light cursor-pointer"
          ...
          <div className={`mt-3 border-1 border-gray-200  lg:h-[7rem] p-2 mb-3 ${dark ? "":"bg-gray-50"}`}>
            {details.map((item) => (
              <div key={item.id}>
          ...
          <div className={`mt-3 ${dark ? "":"bg-gray-50"} border-1 border-gray-200 lg:h-[7rem] p-2 mb-3 `}>
            <aside className="flex justify-between items-center xs:w-[85%] sm:w-[65%] md:w-[60%] lg:w-[40%]">
              <select
                name=""
                id=""
                value={selected}
                className={`xs:p-2 lg:p-3 xs:w-[75%] sm:w-[70%] md:w-[70%] lg:w-[60%] capitalize  font-semibold xs:text-xs sm:text-sm  lg:text-lg transition-all ease duration-200 ${dark ? "bg-[#1A1D25]":"bg-white"}`}
              ...
              >
                ...
                <option value="pay with flutterwave" >pay with flutterwave</option>
              </select>
      ...
      <p className={`uppercase my-8 text-center xs:text-xs md:text-sm transition-all ease duration-200 ${dark ? "text-white":"text-gray-700"}`}>
